...
type
  ...
  private
    ...
    procedure TransmitirCodigoBarras(AReadResult: TReadResult);
  public
    { Public declarations }
  end;
...
procedure TMainForm.TransmitirCodigoBarras(AReadResult: TReadResult);
var
  lJSON: string;
begin
  butStop.OnClick(Self);
  lJSON := dmComunicacao.GerarJSONCarregamento(dmComunicacao.RESTRequest1, AReadResult.text, 1, 1);
  dmComunicacao.ConfigurarConexaoCarregamento(dmComunicacao.RESTClient1, dmComunicacao.RESTRequest1, lJSON);
  dmComunicacao.RESTRequest1.Execute;
  memLog.Lines.Add(FormatDateTime('hh:nn:ss', Now) + ' Transmissão - Status code - ' +
      dmComunicacao.RESTResponse1.StatusCode.ToString);
end;
...
//                  // If send scan to sever  - CPBR 16
                  if SwitchTransmistir.IsChecked then
                    TransmitirCodigoBarras(ReadResult);
